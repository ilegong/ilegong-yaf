...
var check_form = function (formname, opt) {
    ...
    formobj.each(function (i) {
        ...
        if (errflag) {
            ...
            error.push(e_error);
            if (focus_obj == "")
                focus_obj = formobj_i;									//聚焦第一个出现错误对象
            if (this.oldback == undefined)
                this.oldback = this.style.backgroundColor;	//记录原底色
            this.style.backgroundColor = '#FFFF00';									//高亮错误表单对象
        } else {
            if (this.oldback != undefined)
                this.style.backgroundColor = this.oldback;		//还原底色
        }
    });
    //处理ajax校验
    if (!error) {
        for (var i in _temp_ajax) {
            ...
            if (_cf_ajax.call(obj.get(0), obj, str) != true) {
                ...
                error.push(e_error);
                if (focus_obj == "")
                    focus_obj = obj;											//聚焦第一个出现错误对象
                if (obj.get(0).oldback == undefined)
                    obj.get(0).oldback = obj.get(0).style.backgroundColor;	//记录原底色
    ...
    function cf_sameto(obj, el) {
        var str = obj.val();
        if (str == "" || str == formobj.filter("#" + el).val())
            return true;
    ...
    function cf_differentto(obj, el) {
        var str = obj.val();
        if (str == "" || str == formobj.filter("#" + e1).val())
            return true;
