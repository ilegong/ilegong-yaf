...
class Pagination
{
    ...
    protected $maxPagesToShow = 7;

    /**
     * @param int $totalItems 总条数
     * @param int $itemsPerPage 每页条数
     * @param int $currentPage 当前页数
     * @param string $urlPattern 每页的url,(:num)为页码. 如 '/foo/page/(:num)'
     */
    ...
    public function toHtml()
    {
        ...
        $html = '';
        if ($this->getPrevUrl()) {
            $html .= '<a href="' . $this->getPrevUrl() . '">&laquo; 上一页</a>';
        }

        foreach ($this->getPages() as $page) {
            if ($page['url']) {
                if ($page['isCurrent']) {
                    $html .= '<span class="current"> ' . $page['num'] . ' </span>';
                } else {
                    $html .= '<a href=" ' . $page['url'] . '" >' . $page['num'] . ' </a>';
                }
            } else {
                $html .= '<a href="javascript:;">' . $page['num'] . '</a>';
            }
        }

        if ($this->getNextUrl()) {
            $html .= '<a href="' . $this->getNextUrl() . '">下一页 &raquo;</a>';
        }
        //$html .= '</ul>';
